...
def update_cache_for_word(word):
    """
    Searches for all Eitango notes that have the given word,
    and bulk-updates the ExamplesCache field.
    """
    global _is_updating
    if _is_updating: return # Prevent infinite loop
    ...
    try:
        for target_note in notes:
            ...
            for nid, data in note_data.items():
                if nid == target_note.id: continue # Exclude itself
            ...
            if target_note[CACHE_FIELD] != html:
                ...
                col.update_note(target_note) # Save here!
...
def on_editor_unfocus(changed, note, current_field_idx):
    """
    Called when a field loses focus in the editor
    """
...
def update_all_cache():
    """
    Forcibly updates the cache for all Eitango notes
    """
    col = mw.col
