...
/* Accordion section */ /* eitango-css-version: v5 */
.eitango-examples {
	margin-top: 16px;
}
.eitango-section {
...
def parse_words(head_field):
	"""
	Splits the Link field by comma and returns a list of words
...
def get_examples_html(words, current_nid):
	"""
	Generates an accordion section for each Link word.
	Each section can be expanded/collapsed via a ▼ word (count) header.
	"""
	if not words:
...
def update_cache_for_words(words):
	"""
	Updates the cache for all notes that contain any of the given word list
	"""
	global _is_updating
...
def on_js_message(handled, message, context):
	"""
	Handles messages from JS (pycmd)
	"""
